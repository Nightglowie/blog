...
// ils ont : id, visibility, date, title, text, description, tag
let ARTICLES = {
    '1': {id: 1, visibility: true, date: '2018-06-15T17:12:33.962Z', title: 'Insights from Code Conference 2018', text: 'The annual Code Conference continues to be the best place to hear first-hand accounts', 
    ...
    '2': {id: 2, visibility: true, date: '2018-06-05T17:12:33.962Z', title: 'Learning to Trust Machines That Learn',  text: 'Some AI researchers are attempting to address this by creating programs that can essentially keep an eye on their own decision-making processes and describe how they figured things out (known as “explainable AI”)', 
...
class Home extends React.Component {
    ...
    handleIds = (currentIds) => {
        console.log("j'ai reçu les ids" + currentIds)
        let ids = currentIds.slice();
        this.setState({
            idArticles : ids
        });
    }
    ...
    handleVisibility() {
        let i;
        let ids = this.state.idArticles;
        for(i = 0; i<this.ids[i].lenght; i++){
    ...
    render() {
        console.log(this.state.articles[1].visibility)
        return (
            <div> 
                <Actu />
                <SearchBar rechercheIds = {this.handleIds}/>
                {/*Tous les articles*/}
                <Liste
                    articles = {this.state.articles}
                    onChange = {this.handleAfficherContenuArticle}
                    idArticles = {this.state.idArticles}
                />
